...
1. **ALWAYS `cd` into the repo directory before ANY terminal command (git, npm, etc.)**: `cd <PLAYGROUND_ROOT>`
2. **Background terminals start at workspace root** — they MUST use `cd <PLAYGROUND_ROOT> && <command>` as a single command. Never run bare `npm run dev`, `npm run build`, `npm install`, or `git` commands without the `cd` prefix.
